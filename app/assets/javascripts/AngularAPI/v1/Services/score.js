angular.module('ScoreModule',['ApiModelV1Module'])
.service('ScoreSVC',['ApiModelV1','$timeout',function(ApiModelV1,$timeout){
	...
	this.scores = {
		...
		nextPage: function(complete){
			...
			ApiModelV1.query(options,function(data){
				...
				angular.forEach(data.scores,function(val,key){
		...
		find: function(id,complete){
			...
			ApiModelV1.get(options,function(data){
				...
				if (complete){complete(data.score,false);}
		...
		new: function(api_module){
			...
			api_module.update = function(field,value,complete){
				...
				var Item = new ApiModelV1({score: new_item});
				...
				Item.$save(options,function(data){
					...
					api_module.x876[field] = data.score[field];
					api_module[field] = data.score[field];
			...
			api_module.save = function(complete){
				...
				if (ID){
					...
					Item.$save(options,function(data){
						...
						angular.forEach(data.score,function(val,key){
