angular.module('EventModule',['ApiModelV1Module'])
.service('EventSVC',['ApiModelV1','$timeout',function(ApiModelV1,$timeout){
	...
	this.events = {
		...
		find: function(id,complete){
			...
			ApiModelV1.get(options,function(data){
				...
				if (complete){complete(data.event,false);}
		...
		new: function(api_module){
			...
			api_module.update = function(field,value,complete){
				...
				var Item = new ApiModelV1({event: new_item});
				...
				Item.$save(options,function(data){
					...
					api_module.x876[field] = data.event[field];
					api_module[field] = data.event[field];
			...
			api_module.save = function(complete){
				...
				if (ID){
					...
					Item.$save(options,function(data){
						...
						angular.forEach(data.event,function(val,key){
